function [onset, h_onset, h_offset, h_Vonset, h_Voffset] = ...
    changeSpectrogram(AudioTrial, sf, onset, offset, Vonset, Voffset, speed, handles)
...
%     (max(AudioTrial) - min(AudioTrial));
    ...
    [s,f,t,p] = spectrogram(AudioTrial, 128,7/8*128, [], sf, 'power', 'yaxis');
    ...
    if ~isempty(Vonset)
        hold on; h_Vonset = plot(handles.axes3, [Vonset Vonset], ylim, 'g', 'linewidth', 1);
    else
        h_Vonset = [];
    end
    if ~isempty(Voffset)
        hold on; h_Voffset = plot(handles.axes3, [Voffset Voffset], ylim, 'r', 'linewidth', 1);
    else
        h_Voffset = [];
    end
    setappdata(0, 'sf', sf);
